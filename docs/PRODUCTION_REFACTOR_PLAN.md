...
| 14 | ✅ `pydantic-settings` + `.env` secrets loader | 0.5 day | **DONE Phase 12** — `startup_radar/config/secrets.py` exposes `Secrets(BaseSettings)` + lru-cached `secrets()`; `env_prefix="STARTUP_RADAR_"` with `CI` / `SENTRY_DSN` unprefixed aliases; `extra="ignore"` tolerates shell-only vars (`STARTUP_RADAR_SHIP`, etc.). Retired the two surviving `os.getenv` sites in `cli.py:28` + `web/app.py:22`. `.env.example` committed; `tests/conftest.py` autouse-clears the cache between tests. `SENTRY_DSN` field defined but Sentry SDK wiring dropped from the plan (single-user tool — structlog + `runs` + `doctor` cover it). Tag: `phase-12`. |
| 15 | ✅ `httpx.Client` migration + shared HTTP surface | 0.5 day | **DONE Phase 13** — `startup_radar/http.py` exposes `get_client(cfg) -> httpx.Client` (lru-cached per process; `timeout=cfg.network.timeout_seconds`; default `User-Agent: startup-radar/<version>`; `follow_redirects=True`). All three sources (`rss`, `hackernews`, `sec_edgar`) route through it; `sources/rss.py` now fetches bytes via the client and calls `feedparser.parse(r.content)`, retiring the `socket.setdefaulttimeout(20)` module-load hack. Retry tuples updated from `requests.RequestException` → `httpx.HTTPError`. `requests` removed from direct deps (stays transitively via `[google]` extra). `tests/conftest.py` autouse-clears the client cache. Tier 0 #6 closed. Tag: `phase-13`. |
| 16 | ✅ Dockerfile (single image, optional) | 0.5 day | **DONE Phase 14** — multi-stage `python:3.11-slim` image (builder runs `uv sync --frozen --all-extras --no-dev` into `/app/.venv`; runtime ships venv + package source only, no uv, no dev deps); non-root UID 10001; stateless — `/data` + `/config` are the only volumes; single explicit `--config PATH` knob on the top-level Typer callback threaded through all 6 commands (no env-var fallback rats-nest); one scoped hand-off env `STARTUP_RADAR_CONFIG_PATH` that `cli.serve` sets for the Streamlit subprocess and `config/loader.py` is the sole reader; DB path stays in `config.yaml`; `ENTRYPOINT ["startup-radar"]` + default `CMD` serves the dashboard on `:8501`, with `docker run image <args>` letting users override to `… run` / `… doctor`; per-arch image ~204 MB (under 250 MB target); GHCR publishing, `HEALTHCHECK`, and Postgres compose service deferred. Tag: `phase-14`. |
| 17 | ✅ MkDocs site (optional) | 1 day | **DONE Phase 15** — `mkdocs.yml` + Material theme + `include-markdown` + `git-revision-date-localized`; `docs/` built into `site/` via `uv run mkdocs build --strict`; explicit `nav:` (Home, Getting Started, Architecture, Contributing × 2, Sources × 6, Operations × 4, Reasoning trail × 5); `not_in_nav: /plans/*.md` keeps the 17 historical phase plans build-discoverable without polluting top-nav; `exclude_docs: README.md` suppresses the legacy `docs/README.md` → `index.md` conflict; three prose pages authored (`architecture.md` with Mermaid data-flow diagram, `contributing/index.md` + `conventions.md`, `sources/adding-a-source.md`) plus per-source one-pagers; `docs/ops/data-branch.md` → `docs/operations/data-branch.md` via `git mv` with references updated in README, AUDIT_FINDINGS, CLAUDE.md, data-branch-gc workflow, and bootstrap/restore skills; `.github/workflows/docs.yml` builds on every push to `main` that touches `docs/**` / `mkdocs.yml` / `.claude/CLAUDE.md` / `README.md` and publishes via `actions/deploy-pages@v4` (first run requires **Settings → Pages → Source: GitHub Actions**); `make docs` / `make docs-serve` wired; `site/` + `.cache/` gitignored. Tag: `phase-15`. |
| 18 | ✅ Error centralization pass — `StartupRadarError` taxonomy + CLI boundary | 0.5 day | **DONE Phase 16** — three-class taxonomy in `startup_radar/errors.py` (`StartupRadarError` base, `ConfigError`, `SourceError`); `ConfigError` rebased out of `startup_radar/config/loader.py` into `errors.py` with the import re-exported so `from startup_radar.config import ConfigError` still resolves to the same object (identity-preserving back-compat); Typer `@_catch_errors` decorator applied to all six commands (`run`/`serve`/`deepdive`/`doctor`/`status`/`backup`) — catches `StartupRadarError` subclasses, logs `cli.error` via structlog with `command=` / `error=` kwargs, writes a single-line `Error: <msg>` to stderr, and exits 1; `typer.Exit` passes through untouched; generic `Exception` propagates unaltered (bugs stay loud); `--debug` added to the top-level `@app.callback` → threads `exc_info=True` so the `ConsoleRenderer` renders the traceback to stderr; `CLIState` dataclass now carries both `config_path` and `debug`; `sources/base.py` docstring updated with a `SourceError` nudge (no source rewrites — transient failures still follow the `log.warning → return []` pattern per `.claude/rules/observability.md`); audit greps (`raise Exception(`, `except Exception:` without re-raise, `os.getenv`, `logging.basicConfig`, `print(` in library code) reviewed and clean; `.claude/rules/observability.md` + `.claude/CLAUDE.md` gotcha updated; 4 new tests in `tests/unit/test_errors.py` (inheritance, loader re-export identity, boundary one-line-no-traceback, `--debug` traceback rendered). Tag: `phase-16`. |
...
| # | Bug | Location | Fix |
|---|---|---|---|
| 1 | ✅ **FIXED (Phase 7)** GH Actions DB persistence via commit-to-`data`-branch + weekly orphan GC. See `docs/operations/data-branch.md`. |
...
- `Dockerfile` (multi-stage `python:3.11-slim`, non-root UID 10001, stateless) — **landed Phase 14**
- `docker-compose.yml` with `radar` service — **landed Phase 14**; ~~`postgres` service~~ dropped per §0a (single-file SQLite is the contract)
- ~~`.devcontainer/devcontainer.json`~~ deferred (single-user tool; `uv sync` is the dev loop)
