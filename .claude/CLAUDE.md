...
- Docker image is **stateless** — mount `/data` (SQLite DB) and `/config` (`config.yaml` + `credentials.json` + `token.json` + `.env`) from the host (Phase 14). `ENTRYPOINT ["startup-radar"]` + default `CMD` (`--config /config/config.yaml serve --port 8501 --address 0.0.0.0`) means `docker run image` serves the dashboard and `docker run image --config /config/config.yaml run` overrides the args to run the pipeline once; DB path is set in the mounted `config.yaml` (`output.sqlite.path: /data/startup_radar.db`), not via env var. The single controlled env var `STARTUP_RADAR_CONFIG_PATH` is set by `cli.serve` only, to hand its `--config` to Streamlit's child process — `config/loader.py` is the only module that reads it, and it loses to an explicit `load_config(path=...)`. To open a shell inside the image use `--entrypoint bash` (the `docker-shell` Make target does this). Never `docker cp` secrets into a running container; they vanish on recreate.
...
- Exception taxonomy is `startup_radar/errors.py` — three classes (`StartupRadarError` base, `ConfigError`, `SourceError`) (Phase 16). `ConfigError` is re-exported through `startup_radar/config/loader.py` + `startup_radar/config/__init__.py` so existing `from startup_radar.config import ConfigError` call sites still resolve to the same object. The CLI wraps every command in `@_catch_errors` (`cli.py`): it logs `cli.error` via structlog with `command=` / `error=` kwargs, prints a single-line `Error: <msg>` to stderr, and exits 1 for `StartupRadarError` subclasses. The top-level `@app.callback` now takes `--debug`; when set, the decorator passes `exc_info=True` to `log.error` and the `ConsoleRenderer` writes the traceback to stderr. Generic `Exception` propagates untouched (full traceback) — bugs must stay loud. `typer.Exit` is control flow and is re-raised unchanged.
<!-- gotchas:end -->
